...
import { sendTestAlert } from '../../api/alertApi';
...
export default function SettingsPage() {
  ...
  const { userInfo } = useSelector((state) => state.auth);
    ...
    // Mutation for sending the test alert
    const { mutate: testAlert, isPending: isTesting } = useMutation({
        mutationFn: () => sendTestAlert(userInfo.token),
        onSuccess: () => {
      ...
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg max-w-2xl mx-auto">
              ...
              {userInfo?.telegramChatId ? (
                  <div className="space-y-4">
                      <div className="bg-green-900/50 border border-green-700 text-green-300 px-4 py-3 rounded-md relative text-center">
                      ...
                      <p className="text-gray-300">You can now receive price alerts and manage your portfolio from Telegram.</p>

                      <div className="flex space-x-4">
                          <button
                              onClick={testAlert}
                              disabled={isTesting}
                              className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded-md transition-colors w-full disabled:bg-gray-500"
                          >
